...
function makeLegendData(stixIdToObject, config=null)
{
    ...
    for (let object of stixIdToObject.values())
    {
        ...
        // Візуальний тип за замовчуванням = звичайний type
        let visualType = stixType;

    // Спеціальний випадок для location: деталізуємо Country / Region
        if (stixType === "location" && object.has("x_opencti_type")) {
            let subtype = object.get("x_opencti_type");            // "Country", "Region"
            visualType = "location_" + subtype.toLowerCase().replaceAll(" ", "_");
        }
        if (stixType === "threat-actor" && object.has("x_opencti_type")) {
            let taSubtype = object.get("x_opencti_type");

            visualType = "threat_actor_" + taSubtype
            .replaceAll("Threat-Actor-", "")
            .toLowerCase()
            .replaceAll(" ", "_");
        }

        stixTypes.add(visualType);
    }
...
class GraphView extends STIXContentView
{
    ...
    )
    {
        ...
        nodeDataSet.forEach((item, id) => {
            let stixObj = stixIdToObject.get(id);
            let t = stixObj.get("type");

            // Логіка для location
            if (t === "location" && stixObj.has("x_opencti_type")) {
                let subtype = stixObj.get("x_opencti_type"); 
                if (typeof subtype === "string") {
                    t = "location_" + subtype.toLowerCase().replaceAll(" ", "_");
                }
            }
            // Threat-actor subtype handling
            if (t === "threat-actor" && stixObj.has("x_opencti_type")) {
                let taSubtype = stixObj.get("x_opencti_type"); // Threat-Actor-Individual / Threat-Actor-Group
                if (typeof taSubtype === "string") {
                    t = "threat_actor_" + taSubtype
