...
require(["domReady!", "stix2viz/stix2viz/stix2viz"], function (document, stix2viz) {
    ...
    function vizStixWrapper(content, customConfig) {
        ...
        try
        {
            ...
            // --- Видаляємо identity та report з графу ---

            const nodesToRemove = nodeDataSet.get({
                filter: node =>
                    node.id.startsWith("report--") ||
                    node.id.startsWith("identity--")
                    // node.id.startsWith("marking-definition--")
            });
    ...
    function populateLegend(iconURLMap, defaultIconURL) {
        ...
        {
            if (stixType === "marking-definition") continue;
            if (stixType === "identity" || stixType === "report")
                continue;
...
is not serving JSON, or is not running a webserver.\n\nA GitHub Gist can be created to host RAW JSON data to prevent this.");
    ...
    document.getElementById("timelineCheckbox").style.display = "none";
    document.querySelector("label[for='timeline']").style.display = "none";
    let timelineLabel = document.querySelector("label[for='timeline']");
    if (timelineLabel) timelineLabel.style.display = "none";

    let timelineCheckboxLabel = document.querySelector("label[for='timelineCheckbox']");
    if (timelineCheckboxLabel) timelineCheckboxLabel.style.display = "none";


    fetch("Discrediting Ukrainian military personnel.json")   
      .then(r => r.text())
      .then(content => {
        vizStixWrapper(content);  
        linkifyHeader();          
    })
    .catch(err => alert("Помилка завантаження файла: " + err));
});
